...
export default function FormRecord() {
  ...
  const tahapanOptions = useMemo(() => (jenisProduk ? (master.tahapan_by_produk[jenisProduk] || []) : []), [jenisProduk, master.tahapan_by_produk]);

  useEffect(() => {
    (async () => {
      try {
        const [m, todays] = await Promise.all([api.getMaster(), api.listRecords(todayISO())]);
        setMaster(m);
